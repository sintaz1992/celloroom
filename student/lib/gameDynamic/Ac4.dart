...
class Game4 extends ChangeNotifier {
...
// checkcellulorles
  ...
  void targetSet(double targetx, double targety) {
    if ((targetx.toInt() - 1) > -1) {
      ...
      student.acgame4.setx = targetx;
      student.acgame4.sety = targety;
      student.acgame4.targetposition = new Offset(
          maptransferx[targetx.toInt() - 1] * student.playgroundheight,
          maptransfery[targety.toInt() - 1] * student.playgroundheight);
      student.acgame4.targetpathx[activespaceman]
          .add(maptransferx[targetx.toInt() - 1]);
      student.acgame4.targetpathy[activespaceman]
  ...
  void begingame() {
    ...
    tasktext = '';
    student.scoreV = 0;
    student.acgame4.spacemanVisible = [0, 0, 0, 0, 0, 0];
    for (int i = 6; i > 0; i--) {
    ...
    student.acgame4.startgame = false;
    student.reachendV = false;
    startGameV = false;
    student.acgame4.activespaceman = 0;
    student.acgame4.reward = 0;
    student.acgame4.showreport = false;
    student.acgame4.valuelifespaceman = [1, 0, 0, 0, 0, 0];
    student.acgame4.spacemanalive = [
      true,
      true,
      true,
      true,
    ...
    student.acgame4.spacemanpassed = [
      false,
      false,
      false,
      false,
...
/*
    ...
    if (student.groupCurrentTurn != 1) {
      ...
      animationlifespaceman =
          new Timer.periodic(new Duration(seconds: 1), (time) {
        if (!student.showpause) {
          valuelifespaceman[0] = valuelifespaceman[0] - 1 / (timealive);
          //   print(valuelifespaceman);

          if (valuelifespaceman[0] < 2 / (timealive)) {
            valuelifespaceman[0] = 0;
...
*/
    ...
    if (student.acgame4.startGameV) {
      ...
      if (distancePointCenter <=
        ...
        if (student.acgame4.reward < maxreward) {
          if (enterBorderpolygon == false) {
            student.acgame4.reward = student.acgame4.reward + 1;

            student.dbGroupRef.collection("rewards").add({
              "value": student.acgame4.reward,
              "studentName": student.name,
            });
